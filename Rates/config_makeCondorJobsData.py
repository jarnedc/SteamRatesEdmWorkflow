...
inputFilesDir = "/user/jdeclerc/OverlapStudy/SteamRatesEdmWorkflow/Rates/Data_Run2016H"
...
cmsswDir = "/user/jdeclerc/CMSSW_8_0_30_bis/src"

#Json file
json_file = "/user/jdeclerc/OverlapStudy/SteamRatesEdmWorkflow/Rates/lumi_Data_Run2016H.json"
...
if makeInputFilesList:
    command = "python condorScriptForRatesData.py -j %s -e %s -i %s -f %s" %(json_file, cmsswDir, inputFilesDir, file_type)
else:
    command = "python condorScriptForRatesData.py -j %s -e %s -f %s" %(json_file, cmsswDir, file_type)
